...
That workflow requires the search result to remain usable as an object after the search. It must be possible to subset it, combine it, print it, summarize it, update planned replacements, and eventually pass it to `replace_files()`.
...
`seekr` supports this workflow through `as.data.frame` / `as_tibble()` and `as_match()`, so users can move to and from a data frame when that representation is convenient. See the [Tabular workflows article](tabular-workflows.html) for examples.
...
If the user needs full control over reading and writing text, they can use the text-level workflow described in the [Working with text article](working-with-text.html).
